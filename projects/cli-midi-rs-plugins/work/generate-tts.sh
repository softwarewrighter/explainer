...
PROJECT="/Users/mike/github/softwarewrighter/explainer/projects/cli-midi-rs-plugins"
TTS_DIR="$PROJECT/tts"
...
# Use queenbee.local for VoxCPM TTS
TTS_SERVER="http://queenbee.local:7860"
...
for script in $(ls "$SCRIPTS_DIR"/*.txt | sort); do
    ...
    ffmpeg -y -i "$out" -ar 16000 -ac 1 -c:a pcm_s16le /tmp/verify.wav 2>/dev/null
    whisper_out=$(whisper-cli -m "$WHISPER_MODEL" -f /tmp/verify.wav -nt 2>/dev/null)
    echo "  Whisper: ${whisper_out:0:80}..."
    echo ""
done
